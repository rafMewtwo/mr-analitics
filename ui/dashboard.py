"""Dashboard compartilhado — renderiza a análise completa de um PlayerData.

Cada página em pages/ (haechanie, marinao, ...) só carrega o jogador e chama
render(data). Toda a lógica de abas/gráficos vive aqui, sem nome hardcoded.
"""
...
from ui.theme import render_hero, style_plotly
...
def tab_counters(data: PlayerData) -> None:
    ...
    with col1:
        ...
        if not hardest.empty:
            ...
            st.caption("Amostra pequena — sem counter com 3+ encontros ainda.")
    with col2:
        ...
        if not easiest.empty:
            ...
            st.caption("Amostra pequena — sem matchup fácil com 3+ encontros ainda.")
...
def tab_composition(data: PlayerData) -> None:
...
def tab_accuracy(data: PlayerData) -> None:
...
def tab_heroes(data: PlayerData, df: pd.DataFrame) -> None:
...
def render(data: PlayerData) -> None:
    """Renderiza o dashboard completo de um jogador (header + 12 abas)."""
    df = data.matches_df
    name = data.display_name or data.slug

    if df.empty:
        st.error(
            f"Nenhuma partida carregada de `data/{data.slug}/`. "
            f"Rode `python fetch_player.py \"{name}\" --slug {data.slug}` pra baixar os dados."
        )
        st.stop()

    render_hero(
        title=f"{name} · {data.rank}" if data.rank else name,
